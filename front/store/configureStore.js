...
// thunk를 쓰는 이유는 로그인, 로그아웃을 한다고 해서 바로 실행되지 않는다.
// 왜냐하면 백엔드 서버에서 요청을 보내고 응답을 받아야하기 때문이다.
// 10줄의 코드가 끝이다. 한번에 디스패치를 여러번 할 수 있게 해준다. --> thunk의 기능
// 나머지는 스스로 구현해야한다.

// saga를 쓰면 딜레이가 가능하다. 몇 초뒤에 액션이 실행되게 해주는것이 가능하다.
// 복잡한것들 '실수로 클릭을 두번' --> thunk에서는 요청한만큼 다 간다. / saga에서는 takelatest라고 해서 가장 마지막꺼만 요청을 보낸다.
// 셀프 디도스 공격을 막으려면 스로틀을 이용하면 된다. / 디바운스 --> 이런것들을 saga가 미리 구현해둬서 사용한다.
